...
def new_data():
    ...
    for page in pages:
        ...
        for i in ad_links:
            ...
            # get ad price
            try:
                price = soup.find('div', class_='titleRow-4059548442').find_next('span').get_text()
            except AttributeError:
                price = ""

            # get date posted
            try:
                date_posted = soup.find("div", class_="datePosted-383942873").find_next('time')['title']
            ...
            try:
                move_in_date = soup.find('h3', string="Aperçu").find_previous('div').find_next('dt',
                                                                                               string="Date d'emménagement").find_previous(
        ...
        with open("Renting_properties_scraping.csv", 'a+', newline='', encoding="UTF8") as f:
            writer = csv.writer(f)
            writer.writerow(df)
